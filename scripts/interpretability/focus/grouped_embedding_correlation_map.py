...
OUTPUT_DIR = PROJECT_ROOT / "exp_outputs" / "summary" / "grouped_embedding_correlation_maps" / "camcan" / "md"
...
def _plot_all_maps(
    ...
    for (dataset, task), sub_df in embedding_freq.groupby(["dataset", "task"]):
        ...
        plot_brain_and_heatmap(
            ...
            out_file=OUTPUT_DIR / f"brain_heatmap_{dataset}_{task}{suffix}.png"
        )
        ...
        plot_corr_matrix(
            ...
            out_file=OUTPUT_DIR / f"corr_{dataset}_{task}{suffix}.png"
        )
        ...
        plot_corr_matrix(
            ...
            out_file=OUTPUT_DIR / f"binary_agreement_{dataset}_{task}{suffix}.png"
        )
        ...
        plot_corr_matrix(
            acc_matrix2,
            title=f"Binary agreement (≥{THRESHOLD}) | {dataset} | {task}{title_suffix}",
            out_file=OUTPUT_DIR / f"binary_agreement_corrected_{dataset}_{task}{suffix}.png"
        )
        ...
        plot_corr_matrix(
            acc_matrix3,
            title=f"Binary agreement (≥{THRESHOLD}) | {dataset} | {task}{title_suffix}",
            out_file=OUTPUT_DIR / f"binary_agreement_corrected0s_{dataset}_{task}{suffix}.png"
        )
        ...
        plot_corr_matrix(
            ...
            out_file=OUTPUT_DIR / f"binary_agreement_balanced_{dataset}_{task}{suffix}.png"
        )
        ...
        _plot_frequency_rows(
            ...
            OUTPUT_DIR / f"embedding_frequency_maps_{dataset}_{task}{suffix}.png",
        )
...
def main():
    ...
    _plot_all_maps(embedding_freq, global_freq, df, suffix="", title_suffix="")
    _plot_all_maps(
        embedding_freq_net,
        global_freq_net,
        df_network,
    ...
    for dataset, task in global_freq_net[["dataset", "task"]].drop_duplicates().itertuples(index=False):
        plot_region_rank_from_network(
            ...
            dataset,
            task,
            OUTPUT_DIR / f"region_rank_network_{dataset}_{task}.png",
        )

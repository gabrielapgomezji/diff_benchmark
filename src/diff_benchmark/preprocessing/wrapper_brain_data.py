...
class Default_HCPPipeline(DataPreparationBrain):
    ...
    def verify_subject_files(self, subject_id: str, metric: str) -> bool:
        ...
        left_file = derivatives_dir / f"sub-{subject_id}_hemi-L_param-{metric}.scalar.gii"
        right_file = derivatives_dir / f"sub-{subject_id}_hemi-R_param-{metric}.scalar.gii"
    ...
    def run_analysis(self):
        ...
        for left_file in tqdm(scalar_files, desc="Running analysis"):
            try:
                ...
                right_file = left_file.with_name(left_file.name.replace("hemi-L", "hemi-R"))
                ...
                avg_data = average_per_parcel(left_data, right_data, self.schaefer_resampled)
...
class LCOTEmbed_HCPPipeline(DataPreparationBrain):
    def __init__(self, config):
        super().__init__(config)
        self.hcp_dir = Path(config["base_path"])
    ...
    def extract_raw_data(self, subject_id: str):
        ...
        for h in ('L', 'R'):
            ...
            breakpoint()
            nib.save(gii, derivatives_dir / f"sub-{subject_id}_hemi-{h}_raw_dwi.func.gii")
    ...
    def compute_microstructure(self, subject_id: str):        
        ...
        gtab0 = gradient_table(
            bvals=np.zeros(len(sphere.vertices)),
        ...
        for h in ('L', 'R'):
            ...
            rawdwi_dir = self.results_root / "derivatives" / f"sub-{subject_id}" / "dwi" / f"sub-{subject_id}_hemi-{h}_raw_dwi.h5"
        ...
        merged_data["vertex_indices"] = np.array(merged_data["vertex_indices"], dtype=int)

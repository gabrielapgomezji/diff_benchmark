...
\end{figure}
\todo{I moved the figure at the top of the section}
...
different facets of tissue architecture, from basic diffusion magnitude to complex restricted 
environments.\\
\textit{Baseline ($b=0$):} The averaged T2-weighted non-diffusion signal is included as a 
    baseline to disentangle purely diffusion-driven predictive power from underlying structural T2 
    contrast.\\
\textit{Mean Diffusivity (MD):} Computed via Diffusion Tensor Imaging (DTI) \cite{basser1994mr}, representing 
    the overall magnitude of water diffusion.\\
\textit{Mean Kurtosis (MK):} Derived from Diffusion Kurtosis Imaging (DKI) \cite{jensen2005diffusional}, capturing the 
...
\textit{Spherical Harmonics (SH) Power:} A model-free representation obtained by computing the 
...
(e.g., logistic regression, Lasso), Random Forests \todo{Add the quote for random forests (rf paper from 2000 something)}, and Support Vector Machines 
...
training the prediction head. In contrast, the smaller \replace{}{and pretrained} 3D architecture MedicalNet \cite{chen2019med3d} is 

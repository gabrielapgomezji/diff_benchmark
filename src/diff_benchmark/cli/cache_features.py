...
def compute_features_for_dataset(
    ...
    dataset_obj = DatasetConfig(
        **dataset_config,
        base_dir=Path(model_config["cluster"]["paths"][dataset_config["name"]]["base_dir"]),
        results_dir=Path(model_config["cluster"]["paths"][dataset_config["name"]]["results_dir"]),
    )
    ...
    if trainable_params > 0:
        logger.warning(f"WARNING: Backbone has {trainable_params} trainable parameters!")
    ...
    if cache_path.exists() and not force_recompute:
        ...
        if metadata_path.exists():
            ...
            metadata = {
                ...
                'embedding_dim': len([c for c in df.columns if c.startswith('feature_')]),
            }
        
        logger.info(f"Cache contains:")
        logger.info(f"  - Samples: {metadata.get('num_samples', 'unknown')}")
        logger.info(f"  - Augmentations: {metadata.get('num_augmentations', 'unknown')}")
...
def main(cfg: DictConfig) -> None:
    ...
    tissue_type = cfg.dataset.get("tissue_type", None)  # Get tissue type from dataset config
    metric_to_compute = cfg.dataset.get("metric_to_compute", None)  # Get microstructure metric
    ...
    logger.info(f"Microstructure metric: {metric_to_compute if metric_to_compute else 'N/A'}")
    logger.info(f"Normalization: mean={norm_mean}, std={norm_std}")

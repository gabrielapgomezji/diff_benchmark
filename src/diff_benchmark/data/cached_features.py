...
def get_deterministic_rotation_transforms(
    ...
    for _ in range(num_rotations - 1):
        ...
        def make_combined_transform(rotation_angle, do_h_flip, do_v_flip, resize_to=image_size, norm=normalize_transform):
            def transform_fn(x):
                # Normalize first (mandatory)
                x = norm(x)
                # Resize if needed
...
class CachedFeatureDataset(Dataset):
    ...
    def __init__(
        ...
        if image_size is not None and hasattr(image_size, '__iter__'):
            self.image_size = tuple(image_size) if not isinstance(image_size, (list, tuple)) else image_size
        else:
            self.image_size = image_size
        self.norm_mean = norm_mean
        self.norm_std = norm_std
        self.source_dataloader = source_dataloader  # Keep reference for label/gender lookup
    ...
    def _compute_and_cache(
                ...
                for batch_idx, batch in enumerate(dataloader):
                    ...
                    for sample_idx in range(len(x)):
                        ...
                        for aug_idx, transform in enumerate(aug_transforms):
                            ...
                            sample_features.append(features.squeeze(0).cpu())  # (embedding_dim,)
        ...
        file_size_mb = os.path.getsize(self.cache_path) / (1024 ** 2)
        logger.info(f"Saved. Future runs will be instant. (Cache size: {file_size_mb:.2f} MB)")
    ...
    def _save_to_parquet(self, all_features, all_subject_ids):
        ...
        if self.image_size is not None:
            ...
            elif hasattr(self.image_size, '__iter__'):  # Handle ListConfig or other iterables
                image_size_json = list(self.image_size)
            else:
    ...
    def set_augmentation_indices(self, indices: Optional[np.ndarray] = None, mode: str = "random"):
        """
        ...
        if mode == "fixed":
            ...
            self.aug_indices = np.random.randint(0, self.num_augmentations, size=len(self))
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if hasattr(self, '_targets') and hasattr(self, '_genders'):
        ...
        elif self.source_dataloader is not None and hasattr(self.source_dataloader.dataset, '__getitem__'):
            ...
            if len(source_sample) >= 2:
                ...
                gender = source_sample[2] if len(source_sample) >= 3 else torch.tensor(0)
            else:
...
def append_augmentations_to_cache(
    ...
    logger.info(f"New augmentations: {start_aug_idx}-{start_aug_idx + len(new_transforms) - 1}")

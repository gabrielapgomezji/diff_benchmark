...
def plot_network_region_pyramid(
    ...
    out_path = (
        output_dir
        / f"network_region_pyramid_{microstructure}.pdf"
    )
...
def plot_single_network_pyramid(
    ...
    out_path = output_dir / f"pyramid_{network}_{microstructure}.pdf"
...
def plot_network_grid(
    ...
    plt.tight_layout()
    out_path = output_dir / f"network_grid_{microstructure}.pdf"
    plt.savefig(out_path, dpi=300)
    plt.close()

    return out_path

def add_region_metadata(df: pd.DataFrame) -> pd.DataFrame:
    """
    Add:
        - region_id
        - region_name
    ...
    region_to_network = get_region_to_network(surface_atlas)

    # region index extracted from strings such as:
    # "region:123"
    df["region_id"] = (
        df["region"]
        .map(_region_id_from_region_value)
...
df_metadata = add_region_metadata(df)
breakpoint()
all_results = []
...
plt.savefig(
    OUTPUT_DIR
    / f"network_parcel_frequency_{MICROSTRUCTURE}.pdf",
    dpi=300,
)
...
plt.savefig(
    OUTPUT_DIR
    / f"network_touch_frequency_{MICROSTRUCTURE}.pdf",
    dpi=300,
)

    ...
    def project_dwi_to_cortex(self, sub: str):
        ...
        surface_files = {
            "left": {
                ...
                "midthickness": surface_folder
                / f"{sub}.L.midthickness_MSMAll.32k_fs_LR.surf.gii",
            },
            "right": {
                "pial": surface_folder / f"{sub}.R.pial_MSMAll.32k_fs_LR.surf.gii",
                "white": surface_folder / f"{sub}.R.white_MSMAll.32k_fs_LR.surf.gii",
                "midthickness": surface_folder
                / f"{sub}.R.midthickness_MSMAll.32k_fs_LR.surf.gii",
            },
        }
        ...
        required_files = (
            ...
            + list(surface_files["left"].values())
            + list(surface_files["right"].values())
        )
        ...
        try:
            ...
            with h5py.File(raw_data_output, "w") as f:
                for hemi, deen_path in zip(["left", "right"], [deen_left, deen_right]):
                    ...
                    f.create_dataset(
                        f"surface_coordinates_{hemi_key}", data=mesh.coordinates
                    )
...
class DWISchaeferProcessor(RawDataProcessor):
    ...
    def project_dwi_to_cortex(self, sub: str):
        ...
        try:
            ...
            schaefer = (
                self.get_schaefer_labels(scale=schaefer_scale)
                if include_schaefer
                else None
            )
            breakpoint()
            ...
            with h5py.File(raw_data_output, "w") as f:
                for hemi, deen_path in zip(["left", "right"], [deen_left, deen_right]):
                    ...
                    if include_schaefer:
                        f.create_dataset(
                            f"schaefer_labels_{hemi_key}", data=schaefer[hemi]
                        )

    ...
    def __init__(
        ...
        if source_path.exists() and not (source_path / "config.json").exists():
            ...
            if candidates:
                ...
                logger.info(
                    f"Redirecting source to found model directory: {snapshot_dir}"
                )
        ...
        try:
            ...
            self.processor = AutoImageProcessor.from_pretrained(
                source, local_files_only=local_only, trust_remote_code=True
            )
            ...
            logger.info(
                f"Configuring fallback BitImageProcessor using model config: image_size={image_size}"
            )

...
def synthetic_brain_csv(tmp_path):
    ...
    np.random.seed(42)
    subjects = np.random.choice([f"{i:04d}" for i in range(1001, 1021)], size=10, replace=False).tolist()
    n_features = 1000
    ...
    df.insert(0, "subject_id", np.random.choice(subjects, size=len(subjects), replace=False))
...
def synthetic_demographics_csv(tmp_path):
    ...
    df = pd.DataFrame({
        "Subject": subjects,
        "Age_in_Yrs": np.random.randint(18, 70, size=len(subjects)),       # random ages 18-70
        ...
        "Language_Task_Story_Avg_Difficulty_Level": np.random.randint(18, 70, size=len(subjects)),

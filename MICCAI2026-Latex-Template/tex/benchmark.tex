...
\replace{Our proposed benchmark provides a standardized, end-to-end framework for evaluating predictive models 
...
Preprocessing pipelines differ substantially, microstructural information is represented in multiple ways (e.g., voxel-wise maps, ROI summaries, or learned embeddings), and evaluation protocols vary in data splits, cross-validation schemes, and hyperparameter selection.
...
To address these issues, we introduce a standardized benchmark that unifies preprocessing and feature extraction, evaluates both classical and deep learning models on regression and classification tasks, and spans multiple public datasets under a controlled evaluation protocol.
We now describe the benchmark in detail. Its overall structure is illustrated in Figure~\ref{fig:benchmark_overview}.}
...
covering both healthy and clinical populations\replace{, offering diverse demographics, scanner 
types, and acquisition protocols}{}. \replace{For healthy cohorts, we utilize the Human Connectome 
...
{\textbf{HCP}: The Human Connectome Project (HCP) Young Adult dataset \cite{van2013wu} targets gender and age. It includes 1,036 healthy subjects after filtering (474 males, 562 females; ages 22-37).
HCP data were acquired on a customized Siemens 3T Connectome scanner using a multi-shell Spin-Echo EPI sequence (TR/TE = 5520/89.5 ms, 1.25 mm isotropic, multiband factor 3; $b$-values = 1000, 2000, 3000 s/mm$^2$, 90 directions/shell).
\textbf{Cam-CAN}: The Cambridge Centre for Ageing and Neuroscience (Cam-CAN) dataset \cite{shafto2014cambridge} targets gender and age. It includes 629 healthy subjects (316 males, 313 females; ages 18-88), scanned on a Siemens 3T TIM Trio using a 2D twice-refocused SE EPI sequence (TR/TE = 9100/104 ms, 2.0 mm isotropic; $b$-values = 1000, 2000 s/mm$^2$, 30 directions each).
\textbf{ABIDE II}: The Autism Brain Imaging Data Exchange II (ABIDE II) dataset \cite{di2017enhancing} targets Autism Spectrum Disorder (ASD) diagnosis.
It includes 218 subjects (186 males, 32 females; ages 5-64), including 119 with ASD and 99 controls.
As a multi-site dataset, ABIDE II data were acquired on various 3T scanners (e.g., Siemens, GE) with highly heterogeneous, site-specific single-shell diffusion protocols.
Across the included sites, spatial resolutions range from 0.94 to 3.0 mm, TR/TE from 5200-20244/78-101 ms, $b$-values include 1000, 1500, and 2500 s/mm$^2$, and diffusion directions vary between 32 and 64.
...
\replace{\textbf{T1-weighted images using the Desikan–Killiany atlas (aparc)}}{T1-weighted images using the Desikan–Killiany atlas (aparc)}. For each subject, cortical meshes 
...
and residual misalignment, and summarized using the JHU White-Matter Tractography Atlas\todo{Ref?}. Mean 
...
\begin{itemize}
    ...
    L2 norm of the SH coefficients \todo{Ref} (up to order 6) directly from the diffusion signal, capturing signal 
...
% (e.g., ABIDE II), 
We apply a \replace{robust}{} ventricular normalization strategy\todo{Ref}. For each subject, 
...
\replace{To establish a comprehensive evaluation, our}{Our} benchmark \replace{incorporates}{includes} a wide 
spectrum of predictive models, ranging from \replace{simple}{linear} baselines to advanced deep 
learning architectures. \replace{First, we include dummy estimators (majority class for 
classification and mean predictor for regression) to provide a lower-bound 
performance baseline. For classical machine learning, we evaluate}{We first compare} linear models 
(e.g., logistic regression, Lasso)\todo{Ref}, Random Forests\todo{Ref}, and Support Vector Machines 
(SVM)\todo{Ref}. Given the high dimensionality of the microstructural representations, we 
also evaluate these models coupled with Principal Component Analysis (PCA) for 
feature reduction. 

\replace{For deep learning, we}{We also} benchmark several vision architectures. We leverage 
pre-trained \replace{foundation}{} models, specifically DINOv2 \cite{oquab2023dinov2} and Curia \cite{dancette2025curia}, utilizing them as 
frozen feature extractors where representations are computed offline prior to 
training the prediction head. In contrast, \replace{3D architectures like}{the smaller 3D architecture} MedicalNet \cite{chen2019med3d} \replace{are}{is} 
\replace{fully}{} fine-tuned on our diffusion datasets to adapt to the specific 
microstructural domains. \replace{\textbf{To ensure a fair and rigorous comparison, all models 
...
cohorts. \replace{For all classification tasks, binary targets are strictly encoded as 0 and 1. To comprehensively 
assess classification performance, especially in the presence of class imbalances, we compute standard 
and weighted accuracy, precision, recall, and the F1-score.}{We report performance using balanced accuracy, taking into account class imbalances.}
...
\replace{For continuous targets, w}{W}e evaluate brain age estimation using the healthy cohorts from Cam-CAN. 
\replace{During the training of Deep Learning models, these continuous targets are standardized. To prevent data leakage, 
...
and the Pearson correlation coefficient.}{We report performance using R² and MAE.}
...
\replace{This ensures fixed, reproducible train and test splits while maintaining \textbf{the original target distribution 
(e.g., class balance for classification or target range for regression)} across all folds.}
...
subsequently applied to the held-out test set, preventing any information leakage. \replace{Model performance is 

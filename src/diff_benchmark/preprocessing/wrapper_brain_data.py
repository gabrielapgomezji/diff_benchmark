...
class LcotEmbedHcpPipeline(DataPreparationBrain):
    ...
    def compute_microstructure(self, subject_id: str):
        ...
        gtab0 = gradient_table(
            bvals=np.zeros(len(self.sphere.vertices)), bvecs=self.sphere.vertices
        )
        ...
        with h5py.File(output_file, "w") as f:
            ...
            f.attrs["sphere_vertices"] = len(self.sphere.vertices)
    ...
    def compute_embedding(self, subject_id: str):
        ...
        for _, data_split in tqdm(enumerate(split_data(data_torch, 5))):

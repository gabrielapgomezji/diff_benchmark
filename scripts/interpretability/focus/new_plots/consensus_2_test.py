...
def plot_network_cosine_heatmap(network_scores: pd.DataFrame) -> None:
    ...
    fig.tight_layout()
    output_path = PLOTS_DIR / "consensus2_network_cosine_similarity_heatmaps.pdf"
...
def plot_network_barplot(network_scores: pd.DataFrame) -> None:
    ...
    fig.tight_layout()
    output_path = PLOTS_DIR / "consensus2_network_barplot.pdf"
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_score_histogram(scores: pd.DataFrame, item_label: str) -> None:
    ...
    if item_label == "parcels":
        output_path = PLOTS_DIR / "consensus2_average_score_histogram.pdf"
    else:
        output_path = PLOTS_DIR / f"consensus2_{item_label}_average_score_histogram.pdf"
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_top_items(
    ...
    fig.tight_layout()
    if item_label == "parcels":
        output_path = PLOTS_DIR / "consensus2_top20_parcels.pdf"
    else:
        output_path = PLOTS_DIR / f"consensus2_{item_label}_top20.pdf"
    fig.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def print_global_kendall_summary(result: dict) -> None:
    ...
    latex_path = CSV_DIR / "consensus2_networks_global_kendall_table.tex"
...
def plot_score_histogram_per_embedding(
    ...
    for model in REGION_MODELS:
        ...
        output_path = (
            PLOTS_DIR
            / f"per_embedding_histogram_{item_label}_{model}.pdf"
        )
        fig.savefig(output_path, dpi=300, bbox_inches="tight")
        plt.close(fig)
...
def plot_top_items_per_embedding(
    ...
    for embedding in embeddings:
        ...
        if item_label == "parcels":
            output_path = (
                PLOTS_DIR
                / f"consensus2_top{TOP_N_REGIONS}_parcels_{embedding}.pdf"
            )
        else:
            output_path = (
                PLOTS_DIR
                / f"consensus2_{item_label}_top{TOP_N_REGIONS}_{embedding}.pdf"
            )
...
def main() -> None:
    ...
    for item_label, proportions, item_id_column, item_name_column in item_configs:
        ...
        if item_label == "networks":
            ...
            fig.savefig(
                PLOTS_DIR / "consensus2_networks_kendall_table.pdf",

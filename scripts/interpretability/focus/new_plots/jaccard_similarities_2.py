...
def plot_dataset_full_similarity_grid(
    ...
    """
    2×2 cosine similarity grid for one dataset.

    Layout
    ------
    Row 0 (top)    : ElasticNet   | col 0 = all regions | col 1 = all networks
    Row 1 (bottom) : Group Lasso  | col 0 = all regions | col 1 = all networks

    Parameters
    ----------
    data : dict keyed by (model, item_kind) → cosine similarity DataFrame
        Expected keys:
            ("region_elasticnet",  "regions")
            ("region_elasticnet",  "networks")
            ("region_group_lasso", "regions")
            ("region_group_lasso", "networks")
    dataset : str
    metric_label : str   e.g. "Cosine similarity"
    title_prefix : str   e.g. "Cosine similarity"
    output_path  : Path
    """
    sns.set_theme(context="notebook", style="white")
    ...
    for row_idx, model in enumerate(row_models):
        for col_idx, item_kind in enumerate(col_items):
            ...
            sns.heatmap(
                ...
                linecolor="white",
                cbar=draw_cbar,
                cbar_kws={"label": metric_label} if draw_cbar else {},
            )

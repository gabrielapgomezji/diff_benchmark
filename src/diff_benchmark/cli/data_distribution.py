...
def plot_variable_distribution(
    series: pd.Series, output_path: Path, is_target: bool = False
):
    """
...
def generate_summary_report(
    demographics_df: pd.DataFrame, target_columns: list, output_dir: Path
) -> pd.DataFrame:
    """
    ...
    for stats in all_stats:
        summary_row = {
            ...
            "missing": stats["missing"],
            "missing_pct": (
                (stats["missing"] / (stats["count"] + stats["missing"]) * 100)
                if (stats["count"] + stats["missing"]) > 0
                else 0
            ),
        }
    ...
    for target in target_columns:
        ...
        if plot_path.exists():
            ...
            plot_variable_distribution(
                demographics_df[target], plot_path, is_target=True
            )
...
def plot_all_variables(
    demographics_df: pd.DataFrame, output_dir: Path, target_columns: list
):
    """
...
def main(cfg: DictConfig) -> None:
    ...
    if need_full_processing:
        ...
        try:
            ...
            if demographics_df.shape[1] > 50:
                print(
                    f"  Dataset has {demographics_df.shape[1]} columns. Creating summary version."
                )
    ...
    print(f"\nWorkflow tips:")
    print(
        f"  - Change target: Just run again with different target= (plots only new targets)"
    )
